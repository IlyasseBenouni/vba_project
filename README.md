...
We used Alphavantage to extract the data from Yahoo Finance, therefore we are using a free API key generated from Alphavantage. This API key allows us to perform only 5 requests per minute. Hence, we can approximately extract the data from each stock each 12 seconds which makes our data extraction quite long in time. The user has to wait for all the data to be extracted.
